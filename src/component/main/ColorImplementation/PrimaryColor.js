...
const PrimaryColor = () => {
  const [color, setColor] = useState("#133155");
  const [showPicker, setShowPicker] = useState(false);
  const [customColor, setCustomColor] = useState("#133155");
  ...
  return (
    <Box className="border-dashed border border-gray-300 bg-gray-100 p-2 rounded-md mb-10 relative">
      <Box className="flex items-start">
        <input type="hidden" id="BGColor" name="BGColor" value={color} />
        ...
        >
          {/* Dynamically change icon color based on background brightness */}
